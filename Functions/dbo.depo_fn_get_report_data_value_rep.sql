...
CREATE FUNCTION [dbo].[depo_fn_get_report_data_value_rep](@depo_id int, @param_name varchar(max), @is_lat bit)
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF @acc_type = 200
				SET @result = CASE WHEN @is_lat = 1 THEN 'Àtransfer of the existing residue to Plastic Card Account N-' ELSE 'ÀÒÓÄÁÖËÉ ÍÀÛÈÉÓ ÂÀÃÀÒÉÝáÅÀ ÓÀÁÀÒÀÈÄ ÀÍÂÀÒÉÛÆÄ N-' END + CONVERT(varchar(1000), dbo.acc_get_account(@depo_realize_acc_id))
		END
	    ELSE
