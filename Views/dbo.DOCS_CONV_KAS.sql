...
FROM dbo.OPS_0000 D
	INNER JOIN dbo.OPS_0000 B ON B.PARENT_REC_ID = D.REC_ID AND B.DOC_TYPE = 132 
	LEFT JOIN dbo.OPS_0000 C ON C.PARENT_REC_ID = D.REC_ID AND C.DOC_TYPE = 16 
	INNER JOIN dbo.ACCOUNTS AD ON AD.ACC_ID = D.DEBIT_ID
	INNER JOIN dbo.ACCOUNTS AC ON AC.ACC_ID = B.CREDIT_ID
	INNER JOIN dbo.DOC_DETAILS_CONV V ON V.DOC_REC_ID = D.REC_ID
WHERE  D.DOC_TYPE = 122
GO
