...
CREATE PROCEDURE [dbo].[loan_is_good]
	@loan_id int,
	@user_id int,
	@right_name varchar(100) = 'ÍÀáÅÀ',
	@view_name sysname = 'dbo.LOANS',
...
SET @sql_str0 = N'IF EXISTS(SELECT * FROM ' + @view_name + ' L(NOLOCK) '
...
BEGIN
	...
	IF ISNULL(@set_where_sql, '') <> ''
		SET @sql_str = @sql_str + ' WHERE L.LOAN_ID = @loan_id AND ' + @set_where_sql
	ELSE
