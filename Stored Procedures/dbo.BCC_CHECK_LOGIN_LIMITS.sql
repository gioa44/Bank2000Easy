...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[BCC_CHECK_LOGIN_LIMITS] 
  @bc_login_id int,
  @amount TAMOUNT,
...
BEGIN
  ...
  BEGIN
    SET @amount = 1
    IF @lat = 0 
         SET @limit_err_str = '<ERR>ÊËÉÄÍÔÉÓ ËÉÌÉÔÉÓ ÃÀÒÙÅÄÅÀ (ÂÀÃÀÒÉÝáÅÉÓ ÌÀØÓÉÌÀËÖÒÉ ÒÀÏÃÄÍÏÁÀ) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
    ELSE SET @limit_err_str = '<ERR>Excess of client limit (maximum number of transfers) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
  END
  ELSE
  BEGIN
    IF @lat = 0 
         SET @limit_err_str = '<ERR>ÊËÉÄÍÔÉÓ ËÉÌÉÔÉÓ ÃÀÒÙÅÄÅÀ (ÂÀÃÀÒÉÝáÅÉÓ ÌÀØÓÉÌÀËÖÒÉ ÈÀÍáÀ) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
    ELSE SET @limit_err_str = '<ERR>Excess of client limit (maximum amount of transfer) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
  END
  ...
  BEGIN
    ...
    WHERE D.BNK_CLI_ID = @bc_login_id AND D.DOC_DATE = @doc_date
  ...
  BEGIN
    ...
    WHERE D.BNK_CLI_ID = @bc_login_id AND D.DOC_DATE BETWEEN @dt1 AND @dt2
