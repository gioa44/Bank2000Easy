...
SET ANSI_NULLS ON
GO
CREATE  PROCEDURE [dbo].[CONV_INCOME_OUTCOME] AS
DECLARE 
  @dt smalldatetime,
  @dept_no int,
...
WHERE ((A.BAL_ACC_ALT BETWEEN 6000 AND 9999) OR (A.BAL_ACC_ALT=4553))AND A.ISO<>'GEL'-- AND A.SALDO <> $0.00
...
BEGIN
	SELECT	@account=ACCOUNT, @iso=ISO, @branch_id=BRANCH_ID, @bal_acc_alt = BAL_ACC_ALT
	...
	IF @acc_id_c IS NULL
	BEGIN
		PRINT CONVERT(varchar(16), @account) + '/'+ @iso + '  ' + CONVERT(VARCHAR(15), @amount) + ' ÛÄÓÀÁÀÌÉÓÉ ËÀÒÉÓ ÀÍÂÀÒÉÛÉ ÀÒÀÀ ÂÀáÓÍÉËÉ!!!'
		IF @bal_acc_alt = 7075
		BEGIN
			PRINT CONVERT(varchar(16), @account) + '/'+ @iso + '  ' + CONVERT(VARCHAR(15), @amount) + ' ËÀÒÉ ÀÒÀÀ ÖÍÃÀ ÂÀÅÖÛÅÀÈ 7075777 !!!'
			SET @acc_id_c = 8793
		END		
		ELSE
			GOTO NextAcc
