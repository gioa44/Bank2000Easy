...
CREATE PROCEDURE [dbo].[LOAN_SP_RESTORE_OP_LOAN_COLLATERALS2]
	@op_id int,
	@credit_line_id int,
	@op_ext_xml xml OUTPUT
AS
...
FROM dbo.LOAN_FN_GET_COLLATERALS(0, @credit_line_id)
...
SELECT @credit_line_id, COLLATERAL_ID FROM dbo.LOAN_GEN_AGREE_OP_COLLATERALS WHERE OP_ID = @op_id AND IS_LINKED = 1

DELETE dbo.LOAN_COLLATERALS_LINK OUTPUT DELETED.LOAN_ID, DELETED.COLLATERAL_ID INTO @TEMP_LINK1 
WHERE COLLATERAL_ID IN (SELECT COLLATERAL_ID FROM dbo.LOAN_GEN_AGREE_OP_COLLATERALS WHERE OP_ID = @op_id AND ISNULL(IS_LINKED, 0) = 0)
...
SELECT REC_ID, COLLATERAL_ID, MAIN, IS_LINKED FROM dbo.LOAN_GEN_AGREE_OP_COLLATERALS (NOLOCK)
...
BEGIN
	...
	BEGIN
		...
		SELECT 0 AS ROW_VERSION, LOAN_ID, CREDIT_LINE_ID, CLIENT_NO, OWNER, ISO, COLLATERAL_TYPE, AMOUNT, DESCRIP, MARKET_AMOUNT, XML_STR, IS_ENSURED, ENSURANCE_PAYMENT_AMOUNT, ENSUR_PAYMENT_INTERVAL_TYPE, ENSURANCE_COMPANY_ID FROM dbo.LOAN_GEN_AGREE_OP_COLLATERALS
		WHERE REC_ID = @rec_id
		IF @@ERROR <> 0 BEGIN SET @r =1 GOTO proc_end END
...
SET @op_ext_xml = (SELECT * FROM #OLD_COLLATERALS FOR XML RAW, ROOT)
