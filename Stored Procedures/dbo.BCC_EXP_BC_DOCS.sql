...
CREATE PROCEDURE  [dbo].[BCC_EXP_BC_DOCS]
  @bc_client_id int,
  @bc_login_id int,
  @step tinyint,
  @lat bit = 0
...
BEGIN
	...
	DECLARE @flags int
	SELECT @flags = FLAGS FROM dbo.BC_LOGINS (NOLOCK) WHERE BC_LOGIN_ID = @bc_login_id
	...
	WHERE ((@flags & 1024 = 0) OR (D.REC_STATE >= 10) OR (NOT D.DOC_TYPE IN (103,104,113,114))) /* can see red zach */ 
END
...
BEGIN
	...
	FROM dbo.BC_REC_IDS T (NOLOCK)
		INNER JOIN dbo.OPS_0000 D (NOLOCK) ON D.REC_ID = T.REC_ID
...
BEGIN
	...
	FROM dbo.DOC_DETAILS_PLAT DD (NOLOCK) 
		...
		INNER JOIN dbo.OPS_0000 O (NOLOCK) ON O.REC_ID = D.REC_ID
...
BEGIN
	SELECT DD.DOC_REC_ID AS REC_ID,dbo.get_real_bank_code (DD.SENDER_BANK_CODE) AS SENDER_BANK_CODE,
		...
		DD.RECEIVER_BANK_NAME,DD.SENDER_ACC_NAME,DD.RECEIVER_ACC_NAME,DD.INTERMED_BANK_CODE,DD.INTERMED_BANK_NAME,DD.SENDER_TAX_CODE,
		DD.RECEIVER_TAX_CODE,DD.SWIFT_TEXT,DD.REF_NUM,DD.COR_BANK_CODE,DD.COR_BANK_NAME,
...
BEGIN
	SELECT DOC_REC_ID AS REC_ID, 
		 ...
		 P.* , O.CHK_SERIE
	FROM dbo.DOC_DETAILS_PASSPORTS P (NOLOCK)
