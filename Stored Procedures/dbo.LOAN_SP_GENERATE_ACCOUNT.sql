...
SET @prod_code3 = ISNULL(@prod_code3, 0)
SET @loan_no = ISNULL(@loan_no, 0)
...
IF @template = '{OWN_TEMPLATE}' OR @template = '{OWNE_TEMPLATE}'
BEGIN
	EXEC dbo.ON_USER_LOAN_SP_GENERATE_ACCOUNT
...
DECLARE
	...
	@loan_str varchar(20),
    @prod_code3_str varchar(20),
    @bal_acc_str varchar(6),
	@loan_bal_acc_str varchar(6),
    @pos int,
    @length char(1)
...
SET @loan_bal_acc_str = REPLACE(@loan_bal_acc, '.', '')
SET @loan_bal_acc_str = REPLICATE('0', 6 - LEN(@loan_bal_acc_str)) + @loan_bal_acc_str

SET @template = REPLACE(@template, '{CCY}', CASE WHEN @ccy = 'GEL' THEN '0' ELSE '1' END)
SET @template = REPLACE(@template, '{LCCY}', CASE WHEN @loan_ccy = 'GEL' THEN '0' ELSE '1' END)
...
SET @pos = PATINDEX ('%{L_}%', @template)
IF @pos > 0
BEGIN
	SET @length = SUBSTRING(@template, @pos + 2, 1)
	SET @loan_str = RIGHT(CONVERT(varchar(20), @loan_no), @length)
	SET @template = REPLACE(@template, '{L' + @length + '}', REPLICATE('0', @length - LEN(@loan_str)) + @loan_str)
	SET @pos = PATINDEX ('%{L_}%', @template)
END
...
BEGIN
	SET @length = SUBSTRING(@template, @pos + 2, 1)
	SET @prod_code3_str = RIGHT(CONVERT(varchar(20), @prod_code3), @length)
	SET @template = REPLACE(@template, '{P' + @length + '}', REPLICATE('0', @length - LEN(@prod_code3_str)) + @prod_code3_str)
	SET @pos = PATINDEX ('%{P%}%', @template)
END
...
SET @pos = PATINDEX ('%{LA_}%', @template)
WHILE @pos > 0
BEGIN
	SET @length = SUBSTRING(@template, @pos + 3, 1)
	SET @pos = CONVERT(int, @length)
	SET @template = REPLACE(@template, '{LA' + @length + '}', SUBSTRING(@loan_bal_acc_str, @pos, 1))
