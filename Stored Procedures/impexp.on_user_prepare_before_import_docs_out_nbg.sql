...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [impexp].[on_user_prepare_before_import_docs_out_nbg]
	@date smalldatetime,
	@por int,
	@user_id int,
	@is_close_day bit
...
BEGIN
	SET @amount_d = dbo.acc_get_balance(@acc_id_f, @date, DEFAULT, DEFAULT, 2)	
	IF @amount_d >= $0.00 GOTO _next

	SET @amount_d = - @amount_d
	SET @amount_c = ROUND(dbo.get_equ(@amount_d, @iso_f, @date), 2) 
...
WHILE @@FETCH_STATUS = 0
BEGIN
	SET @amount_d = dbo.acc_get_balance(@acc_id_n, @date, DEFAULT, DEFAULT, 2)	
	IF @amount_d >= $0.00 GOTO _next_1_2
	SET @amount_d = - @amount_d

	SET @amount_c = dbo.acc_get_balance(@tax_revert_acc_id, @date, DEFAULT, DEFAULT, 2)
...
SELECT @descrip = DESCRIP FROM dbo.SAXAZCODES_4 (NOLOCK) WHERE ID = SUBSTRING(@saxazkod, 6, 4)
...
WHILE @@FETCH_STATUS = 0
BEGIN
	SET @amount = dbo.acc_get_balance(@debit_id, @date, DEFAULT, DEFAULT, 2)	
