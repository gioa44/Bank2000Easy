...
CREATE PROCEDURE [dbo].[LOAN_SP_LOAN_PROCESSING_RISKS_GROUPS]
	@date smalldatetime,
	@user_id int,
	@loan_id int = NULL
AS
...
FOR SELECT GROUP_ID
FROM dbo.LOANS
WHERE [STATE] <> dbo.loan_const_state_closed() AND GROUP_ID IS NOT NULL AND
...
FETCH NEXT FROM cr_c INTO @group_id
...
BEGIN
      ...
      WHERE L.GROUP_ID IS NOT NULL AND L.GROUP_ID = @group_id AND L.[STATE] <> dbo.loan_const_state_closed()
...
--                SET @max_category_level_2 = @cr_l_max_category_level
      ...
      WHERE L.GROUP_ID = @group_id AND L.[STATE] <> dbo.loan_const_state_closed() /*AND L.LOAN_TYPE = 1*/ AND D.MAX_CATEGORY_LEVEL <> @max_category_level_2 AND O.OP_ID IS NULL
...
/*    IF @max_category_level < @max_category_level_2
      ...
      FROM dbo.LOANS L
            INNER JOIN dbo.LOAN_DETAILS D ON D.LOAN_ID = L.LOAN_ID
            LEFT OUTER JOIN   dbo.LOAN_OPS O ON O.LOAN_ID = L.LOAN_ID AND O.OP_TYPE = dbo.loan_const_op_restructure_risks()
      WHERE L.GROUP_ID IS NOT NULL AND L.GROUP_ID = @group_id AND L.[STATE] <> dbo.loan_const_state_closed() AND L.LOAN_TYPE <> 1 AND MAX_CATEGORY_LEVEL <> @max_category_level AND O.OP_ID IS NULL
*/
      FETCH NEXT FROM cr_c INTO @group_id
END
