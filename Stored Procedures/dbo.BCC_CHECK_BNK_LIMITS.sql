...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[BCC_CHECK_BNK_LIMITS] 
  @bc_client_id int,
  @amount TAMOUNT,
...
SELECT LIMIT_TYPE, LIMIT_PERIOD, LIMIT_VALUE
FROM dbo.BC_BNK_LIMITS
...
BEGIN
  ...
  BEGIN
    SET @amount = 1
    IF @lat = 0 
         SET @limit_err_str = '<ERR>ÁÀÍÊÉÓ ËÉÌÉÔÉÓ ÃÀÒÙÅÄÅÀ (ÂÀÃÀÒÉÝáÅÉÓ ÌÀØÓÉÌÀËÖÒÉ ÒÀÏÃÄÍÏÁÀ) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
    ELSE SET @limit_err_str = '<ERR>Excess of bank limit (maximum number of transfers) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
  END
  ELSE
  BEGIN
    IF @lat = 0 
         SET @limit_err_str = '<ERR>ÁÀÍÊÉÓ ËÉÌÉÔÉÓ ÃÀÒÙÅÄÅÀ (ÂÀÃÀÒÉÝáÅÉÓ ÌÀØÓÉÌÀËÖÒÉ ÈÀÍáÀ) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
    ELSE SET @limit_err_str = '<ERR>Excess of bank limit (maximum amount of transfer) - [' + ltrim(str(@limit_period)) + ':' + ltrim(str(@limit_value)) +  ']</ERR>'
  END
  ...
  BEGIN
    ...
    FROM dbo.OPS_0000 D (NOLOCK)
		INNER JOIN dbo.BC_LOGINS L (NOLOCK) ON L.BC_LOGIN_ID = D.BNK_CLI_ID
    WHERE L.BC_CLIENT_ID = @bc_client_id AND D.DOC_DATE = @doc_date
  ...
  BEGIN
    ...
    FROM dbo.OPS_FULL D (NOLOCK)
		INNER JOIN dbo.BC_LOGINS L (NOLOCK) ON L.BC_LOGIN_ID = D.BNK_CLI_ID
    WHERE L.BC_CLIENT_ID = @bc_client_id AND D.DOC_DATE BETWEEN @dt1 AND @dt2
