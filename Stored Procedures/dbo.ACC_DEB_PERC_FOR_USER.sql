...
CREATE PROCEDURE [dbo].[ACC_DEB_PERC_FOR_USER]
  @user_id int,
  @all_recs bit = 0,
  @dt smalldatetime
AS
...
BEGIN
  SET @sql_str = 
  'SELECT A.ACCOUNT, A.ISO, P.*,$0.0000 AS TAX_RATE FROM dbo.ACCOUNTS_DEB_PERC P(NOLOCK) INNER JOIN dbo.ACCOUNTS A(NOLOCK) ON A.ACC_ID=P.ACC_ID
...
BEGIN
  ...
  SET @sql_str = 
  'SELECT A.ACCOUNT, A.ISO, P.* FROM dbo.ACCOUNTS_DEB_PERC P(NOLOCK) INNER JOIN dbo.ACCOUNTS A(NOLOCK) ON A.ACC_ID=P.ACC_ID AND
