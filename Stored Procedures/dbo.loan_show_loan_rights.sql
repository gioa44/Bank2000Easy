...
CREATE PROCEDURE [dbo].[loan_show_loan_rights]
	@loan_id int,
	@user_id int
AS
...
SET @sql_str0 = N'IF EXISTS(SELECT * FROM dbo.LOANS L(NOLOCK) '
...
FROM dbo.loan_user_sets_sql(@user_id, null)
...
BEGIN
	...
	IF ISNULL(@where_sql, '') <> ''
		SET @sql_str = @sql_str + ' WHERE L.LOAN_ID = @loan_id AND ' + @where_sql
	ELSE
		SET @sql_str = @sql_str + ' WHERE L.LOAN_ID = @loan_id'
	
	SET @sql_str = @sql_str + ' ) SET @is_ok = 1'
	
	--PRINT @sql_str
	SET @is_ok = 0
	EXEC sp_executesql @sql_str, N'@loan_id int, @branch_id int, @dept_no int, @user_id int, @is_ok bit OUTPUT', @loan_id, @branch_id, @dept_no, @user_id, @is_ok OUTPUT
