...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [impexp].[doc_generate_revert_in_nbg]
	@date smalldatetime,
	@por int,
	@row_id int,
	@uid int,
...
SELECT @doc_rec_id = DOC_REC_ID, @finalyze_doc_rec_id = FINALYZE_DOC_REC_ID, @old_uid = UID, @acc_id = ACC_ID
...
BEGIN
	...
	BEGIN
		...
		IF @rec_state >= 10
		BEGIN
			RAISERROR ('ÀÌ ÓÀÁÖÈÉÓ ÛÄÓÀÁÀÌÉÓÉ ÖÊÀÍ ÃÀÓÀÁÒÖÍÄÁÄËÉ ÓÀÂÀÃÀÓÀáÀÃÏ ÃÀÅÀËÄÁÀ ÀÅÔÏÒÉÆÄÁÖËÉÀ. ÓÀÁÖÈÉÓ ÂÄÍÄÒÀÝÉÉÓ ÛÄØÝÄÅÀ ÀÒ ÛÄÉÞËÄÁÀ.', 16, 1)
			IF @@TRANCOUNT > 0 ROLLBACK 
			RETURN 1
		END
...
SET STATE = 21, DOC_REC_ID = NULL, FINALYZE_DOC_REC_ID = NULL, ACC_ID = @acc_id, UID = UID + 1
WHERE PORTION_DATE = @date AND PORTION = @por AND ROW_ID = @row_id
IF @@ERROR <> 0 BEGIN ROLLBACK RETURN 1 END
