...
CREATE PROCEDURE [impexp].[doc_reject_revert_in_nbg]
	@date smalldatetime,
	@por int,
	@row_id int,
	@uid int,
...
BEGIN
	...
	IF @rec_state >= 10
	BEGIN
		RAISERROR ('ÀÌ ÓÀÁÖÈÉÓ ÛÄÓÀÁÀÌÉÓÉ ÓÀÊÏÒÄÓÐÏÍÃÄÍÔÏ ÀÍÂÀÒÉÛÉÓ ÂÀÌÀÓßÏÒÄÁÄËÉ ÂÀÔÀÒÄÁÀ ÀÅÔÏÒÉÆÄÁÖËÉÀ. ÓÀÁÖÈÉÓ ÀÙÃÂÄÍÀ ÀÒ ÛÄÉÞËÄÁÀ.', 16, 1)
...
SET STATE = 29, DOC_REC_ID = NULL, UID = UID + 1
WHERE PORTION_DATE = @date AND PORTION = @por AND ROW_ID = @row_id
IF @@ERROR <> 0 BEGIN ROLLBACK RETURN 1 END
