...
CREATE PROCEDURE [dbo].[CLEARING_OUTBAL]
	@dt smalldatetime,
	@head_branch_id int
AS
...
BEGIN
	SET @d_acc_id = dbo.acc_get_acc_id(@head_branch_id, dbo.get_clearing_account(@d_branch_id, @ccy, 0, 0), @ccy)
	SET @c_acc_id = dbo.acc_get_acc_id(@d_branch_id, dbo.get_clearing_account(@d_branch_id, @ccy, 1, 0), @ccy)
...
WHILE @@FETCH_STATUS = 0
BEGIN
	SET @d_acc_id = dbo.acc_get_acc_id(@c_branch_id, dbo.get_clearing_account(@c_branch_id, @ccy, 1, 0), @ccy)
	SET @c_acc_id = dbo.acc_get_acc_id(@head_branch_id, dbo.get_clearing_account(@c_branch_id, @ccy, 0, 0), @ccy)
