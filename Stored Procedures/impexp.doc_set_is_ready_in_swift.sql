...
CREATE PROCEDURE [impexp].[doc_set_is_ready_in_swift]
	@date smalldatetime,
	@por int,
	@row_id int,
	@uid int,
...
SELECT @rec_uid = [UID], @old_is_ready = IS_READY
...
SET UID = UID + 1, IS_READY = @is_ready
WHERE PORTION_DATE = @date AND PORTION = @por AND ROW_ID = @row_id
IF @@ERROR <> 0 BEGIN ROLLBACK RETURN 1 END
...
VALUES (@date, @por, @row_id, @user_id, 32 - @is_ready, 'ÓÀÁÖÈÉÓ ÌÆÀÃÚÏ×ÍÉÓ ÛÄÝÅËÀ: ' + CONVERT(varchar(10), @old_is_ready) + ' -> ' + CONVERT(varchar(10), @is_ready))
IF @@ERROR <> 0 BEGIN ROLLBACK RETURN 1 END
