...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[cli_show_client_rights]
	@client_id int,
	@user_id int
AS
...
SET @sql_str0 = N'IF EXISTS(SELECT * FROM dbo.CLIENTS C(NOLOCK) '
...
FROM dbo.cli_user_sets_sql(@user_id, null)
...
BEGIN
	...
	IF ISNULL(@where_sql, '') <> ''
		SET @sql_str = @sql_str + ' WHERE C.CLIENT_NO = @client_id AND ' + @where_sql
	ELSE
		SET @sql_str = @sql_str + ' WHERE C.CLIENT_NO = @client_id'
	
	SET @sql_str = @sql_str + ' ) SET @is_ok = 1'
	
	--PRINT @sql_str
	SET @is_ok = 0
	EXEC sp_executesql @sql_str, N'@client_id int, @branch_id int, @dept_no int, @user_id int, @is_ok bit OUTPUT', @client_id, @branch_id, @dept_no, @user_id, @is_ok OUTPUT
