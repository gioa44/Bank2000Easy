...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[cli_show_clients]
	@user_id int,
...
SET @sql_str0 = N'SELECT C.CLIENT_NO FROM C '
SET @sql = ''
...
ELSE
IF @field_list <> 'CLIENT_NO' AND @field_list <> 'C.CLIENT_NO'
BEGIN
	SET @sql = N'SELECT ' +  @topn + @field_list + N' FROM (' + CHAR(13) +
		@sql + N') CCC' + CHAR(13) +
		N'  INNER JOIN ' + @view_name + N' C (NOLOCK) ON C.CLIENT_NO = CCC.CLIENT_NO'
END

SET @sql = N'WITH C AS (SELECT C.* FROM dbo.CLIENTS C(NOLOCK)' + @join_sql +
