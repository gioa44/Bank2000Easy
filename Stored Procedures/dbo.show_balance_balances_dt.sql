...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[show_balance_balances_dt]
	@start_balance  bit = 0,
	@dt smalldatetime,
	@iso TISO = '***',
	@equ bit = 1,
...
ELSE 
IF @oob = 2 SET @oob_where_str = N' AND A.BAL_ACC<1000'
ELSE
  SET @oob_where_str = N''

IF @shadow_level >= 0 AND @dt < dbo.bank_open_date()
	SET @shadow_level = -1
...
SET @sql = @sql + 
	N'SELECT SUM(A.DBS) AS DBS,SUM(A.CRS) AS CRS,' + @group_by + char(13) +
	N'FROM A' + char(13) +
	N' INNER JOIN @branches B ON B.[ID] = A.BRANCH_ID' + char(13)
	
SET @sql = @sql + 
	N'WHERE A.DT = @dt' + @oob_where_str + @iso_where_str + char(13) +
	N'GROUP BY ' + @group_by 
...
--print @sql
EXEC sp_executesql @sql, N'@dt smalldatetime,@branch_list varchar(255),@user_id int',@dt, @branch_str, @user_id
GO
