...
CREATE PROCEDURE [dbo].[acc_show_acc_rights]
	@acc_id int,
	@user_id int
AS
...
SET @sql_str0 = N'IF EXISTS(SELECT * FROM dbo.ACCOUNTS A(NOLOCK) '
...
FROM dbo.acc_user_sets_sql(@user_id, null)
...
BEGIN
	...
	IF ISNULL(@where_sql, '') <> ''
		SET @sql_str = @sql_str + ' WHERE A.ACC_ID = @acc_id AND ' + @where_sql
	ELSE
