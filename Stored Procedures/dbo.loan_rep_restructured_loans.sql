...
CREATE PROCEDURE [dbo].[loan_rep_restructured_loans] (
	@user_id int,
...
BEGIN
	INSERT INTO #tbl1
	SELECT LD.LOAN_ID, LD.RESTRUCTURED, LD.RAITING 
	FROM dbo.loan_get_loan_data(@loan_id, @date) LD
	WHERE LD.RESTRUCTURED > 0
...
SET @sql = N'
SELECT ' + @field_list + N', R.RESTRUCTURED AS R_RESTRUCTURED, R.RAITING AS RAITING, LO.OP_DATE
FROM ' + @view_name + N' L
	INNER JOIN #tbl1 R ON R.LOAN_ID = L.LOAN_ID	
	INNER JOIN dbo.LOAN_OPS LO (NOLOCK) ON LO.LOAN_ID = L.LOAN_ID
WHERE L.STATE <> 255 AND LO.OP_ID = (SELECT MAX(OP_ID)
									 FROM dbo.LOAN_OPS LO2 (NOLOCK) 
									 WHERE LO2.LOAN_ID = L.LOAN_ID 
									 AND LO2.OP_TYPE IN ( dbo.loan_const_op_prolongation(), dbo.loan_const_op_restructure(), dbo.loan_const_op_restructure_params(), dbo.loan_const_op_restructure_risks() ) 
