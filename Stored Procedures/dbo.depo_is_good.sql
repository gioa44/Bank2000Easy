...
CREATE PROCEDURE [dbo].[depo_is_good]
	@depo_id int,
	@user_id int,
	@right_name varchar(100) = 'ÍÀáÅÀ',
	@view_name sysname = 'dbo.DEPOS',
...
SET @sql_str0 = N'IF EXISTS(SELECT * FROM ' + @view_name + ' D(NOLOCK) '
...
FROM dbo.depo_user_sets_sql(@user_id, @right_name)
...
BEGIN
	...
	IF ISNULL(@set_where_sql, '') <> ''
		SET @sql_str = @sql_str + ' WHERE D.DEPO_ID = @depo_id AND ' + @set_where_sql
	ELSE
