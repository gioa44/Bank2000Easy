...
CREATE PROCEDURE [dbo].[RP_GET_BAL_ACC_BALANCE]
 @dt smalldatetime,
 @bal_acc TBAL_ACC,	
 @equ bit = 0,
 @branch_id int = -1,
...
FROM dbo.TMP_BALANCE_DETAILS 
WHERE CALC_DATE = @dt AND BRANCH_ID = @branch_id 

IF (@@ROWCOUNT = 0)
	TRUNCATE TABLE dbo.TMP_BALANCES
...
BEGIN
	...
	UPDATE dbo.TMP_BALANCE_DETAILS
	SET CALC_DATE = @dt, CALC_FLAGS = @calc_flags | @flag, BRANCH_ID = @branch_id

	IF (@@ROWCOUNT = 0)
		INSERT INTO dbo.TMP_BALANCE_DETAILS
