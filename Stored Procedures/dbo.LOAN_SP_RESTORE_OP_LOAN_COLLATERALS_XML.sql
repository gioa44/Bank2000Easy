...
CREATE PROCEDURE [dbo].[LOAN_SP_RESTORE_OP_LOAN_COLLATERALS_XML]
	@op_id int,
	@loan_id int
AS
...
SELECT @loan_id, COLLATERAL_ID FROM dbo.LOAN_VW_OP_LOAN_COLLATERALS WHERE OP_ID = @op_id AND IS_LINKED = 1

DELETE dbo.LOAN_COLLATERALS_LINK OUTPUT DELETED.LOAN_ID, DELETED.COLLATERAL_ID INTO @TEMP_LINK1 
WHERE COLLATERAL_ID IN (SELECT COLLATERAL_ID FROM dbo.LOAN_VW_OP_LOAN_COLLATERALS WHERE OP_ID = @op_id AND ISNULL(IS_LINKED, 0) = 0)
...
SELECT COLLATERAL_ID, MAIN, IS_LINKED FROM dbo.LOAN_VW_OP_LOAN_COLLATERALS (NOLOCK)
...
BEGIN
	...
	BEGIN
		...
		SELECT 0 AS ROW_VERSION, LOAN_ID, CREDIT_LINE_ID, CLIENT_NO, OWNER, ISO, COLLATERAL_TYPE, AMOUNT, DESCRIP, MARKET_AMOUNT, XML_STR, IS_ENSURED, ENSURANCE_PAYMENT_AMOUNT, ENSUR_PAYMENT_INTERVAL_TYPE, ENSURANCE_COMPANY_ID FROM dbo.LOAN_VW_OP_LOAN_COLLATERALS
