...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[DX_SPX_GET_DEPOSIT_LIST_PORTFILIO]
  @officer_id int,
...
SET @sql_str = 'SELECT * FROM dbo.DX_VW_DEPOSIT_LIST_PORTFILIO A (NOLOCK)'

SET @where_sql = ISNULL(@where_sql, '')

IF @officer_id IS NOT NULL
  SET @sql_str = @sql_str + N' WHERE (A.OFFICER_ID=@officer_id OR A.OFFICER_ID IN (SELECT USER_ID_2 FROM USER_RELATIONS WHERE USER_ID=@officer_id AND FLAGS & 8 <> 0))'
