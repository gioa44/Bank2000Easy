...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[cli_is_good]
	@client_id int,
	@user_id int,
	@right_name varchar(100) = 'ÍÀáÅÀ',
	@view_name sysname = 'dbo.CLIENTS',
...
SET @sql_str0 = N'IF EXISTS(SELECT * FROM ' + @view_name + ' C(NOLOCK) '
...
FROM dbo.cli_user_sets_sql(@user_id, @right_name)
...
BEGIN
	...
	IF ISNULL(@set_where_sql, '') <> ''
		SET @sql_str = @sql_str + ' WHERE C.CLIENT_NO = @client_id AND ' + @set_where_sql
	ELSE
